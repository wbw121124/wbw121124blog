        ...
        md.use(markdownItContainer, containerType, {
            ...
            render(tokens, idx) {
                ...
                if (token.nesting === 1) {
                    // 处理可折叠容器 (details)
                    if (type === 'details') {
                        const summary = title || '详细信息';
                        return `<details ${openAttr} class="custom-container ${type}"><summary>${md.utils.escapeHtml(summary)}</summary>\n`;
                    }
                    // 处理普通提示框
                    const header = title ? `<div class="custom-container-header">${md.utils.escapeHtml(title)}</div>` : '';
