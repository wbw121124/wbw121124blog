		...
		const katexRenderer = {
			inlineRenderer: (str) => {
				try {
					...
					return `<span role="text" aria-label="${escapeA11yText(a11yText)}">${html}</span>`;
				} catch (error) {
			...
			blockRenderer: (str) => {
				try {
					...
					const a11yText = getSafeA11yLabel(str, katex, katexOptions);
					return `<span role="text" style="overflow-x: auto;" aria-label="${escapeA11yText(a11yText)}">${html}</span>`;
				} catch (error) {
