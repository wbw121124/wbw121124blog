	...
	while (i < n) {
		...
		if (ch === '<') {
			...
			for (let j = 0; j < stack.length; j++) {
				result += `<${stack[j].tag}${stack[j].class}>`;
			}
			i++;
		} else {
			// 普通字符
...
export default md => {
	...
	md.renderer.rules.fence = (...args) => {
		...
		let code = '';
		if (langName != 'cpp') {
			code = highlight(token.content, langName)
		}
		else {
			code = caster.toHighlightJS(token.content);
		}
